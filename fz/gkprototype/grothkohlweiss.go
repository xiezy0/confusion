...
func prove(ring Ring, sk *big.Int) {
	...
	for j := 0; j < n; j++ {
		...
		// clj = lj * h + rj * g
		commitments = append(commitments, commit(big.NewInt(int64(signerindexbin[j])), randomvars[5*j]))
		...
		// cbj = (lj * aj) * h + tj * g
		signnn := new(big.Int).Add(big.NewInt(int64(signerindexbin[j])),randomvars[5*j+1])
		commitments = append(commitments, commit(signnn, randomvars[5*j+3]))
		...
		for i := 0; i < N; i++ {
			...
			temp := ring.PubKeys[j].ScalarMult(polycoeff[i][k])
			product = product.Add(temp)
		}


		cdk := product.Add(CurvePoint{}.ScalarBaseMult(rhok))
